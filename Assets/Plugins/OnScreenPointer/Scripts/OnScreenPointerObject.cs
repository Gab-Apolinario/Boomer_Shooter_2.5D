...
namespace OnScreenPointerPlugin
{
    public class OnScreenPointerObject : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                Vector3 directionToTarget = transform.position - camera_local.transform.position;
                float distanceToTarget = directionToTarget.magnitude;

                if (Physics.Raycast(camera_local.transform.position, directionToTarget, out RaycastHit hit, distanceToTarget))
                {
                    if (hit.collider.gameObject != gameObject)
                    {
                        isPointerInScreen = false;
                    }
                }
            }
